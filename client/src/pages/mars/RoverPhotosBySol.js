...
import { Typography, TextField, Button, CircularProgress, Grid, Container, Box, FormControl, InputLabel, Select, Modal, } from '@material-ui/core';
import IconButton from '@material-ui/core/IconButton';
import { useStyles } from '../../stylesheets/marsstylesheets/SolRoverStyles';
...
function RoverPhotosBySol() {
  ...
  const handleFetchPhotos = async () => {
    try {
      setLoading(true);
      const response = await fetchRoverPhotosBySol(rover, sol, camera || null, pages); // Pass the selected number of pages
  ...
  return (
    <div className={classes.root}>
      ...
      <Grid container spacing={2} className={classes.inputContainer}>
        <Grid item xs={12}>
          <Typography variant="h3" className={classes.title}>Mars Rover Photos by Sol</Typography><br />
        </Grid>
        ...
        <Grid item xs={12}>
          <Button variant="contained" color="primary" onClick={handleFetchPhotos} disabled={!sol || loading}>
