...
export default function Navbar({ title = "NASA API PORTAL", logged }) {
  const classes = useStyles();
  const navigate = useNavigate();

  return (
    <AppBar position="absolute" className={`${classes.appBar} ${logged ? classes.loggedInAppBar : classes.loggedOutAppBar}`}>
      <Toolbar>
        <IconButton component={RouterLink} to="/home" edge="start" color="inherit" aria-label="home">
          <img src={logo} alt="Logo" className={classes.logo} />
        </IconButton>
